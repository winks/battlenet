...
(defn read-remote-profile
  ...
  (let [url (create-url-profile region defs/bn-game-d3 bn-path-profile profile number params)]
    (read-str (read-url url) :key-fn keyword)))

(defn read-remote-hero
  "Returns hero data as a map."
  [region hero params]
  (let [url (create-url-hero region defs/bn-game-d3 bn-path-hero hero params)]
    (read-str (read-url url) :key-fn keyword)))
